...
///
/// <para>
/// Донайм ограничен <see cref="MaxWorkersMultiplier"/> и нанимаем РОВНО под потребность, не больше:
/// сверх потребности численность не наращивается вовсе, у уровня без внутренних потребителей
/// (конечный продукт) план остаётся базовым.
///
/// <para>
/// Уточнение 2026-09-07 (<c>docs/levers.md</c>, проверено <c>OverStaffingEconomicsTests</c> на живой
/// цепочке): прежняя формулировка «под наценкой лишний рабочий формально всегда выгоден» была
/// неверна. Цена продажи считается <see cref="MaterialCostCalculator"/> по ЭТАЛОННОЙ фабрике с
/// <see cref="WorkerProductivityConfig.BaseWorkerCount"/> рабочих и от численности конкретной фабрики
/// не зависит, поэтому лишний рабочий приносит выручку через выпуск с убывающей отдачей, а зарплату
/// тянет линейно. Знак эффекта — знак <c>0.15·сырьё + 0.65·содержание + 0.15·электричество −
/// 0.35·зарплата</c>: на дешёвых сырьевых фабриках перенайм режет прибыль, на глубоких переделах
/// добавляет. Ограничение здесь всё равно нужно, но по смыслу («дальше расширяемся новой фабрикой»),
/// а не как защита от вырожденности ценообразования.
/// </para>
/// </para>
/// </summary>
