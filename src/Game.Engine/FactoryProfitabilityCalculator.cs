...
public static class FactoryProfitabilityCalculator
{
        ...
        decimal electricityConsumptionPerOutputUnit = 0m)
    {
        ...
        // сама командная зарплата тоже плоская (прогрессия сверх порога убрана целиком) — при
        // флэт-ставке это исключение уже не меняет итоговую долю остальных фабрик (она
        // пропорциональна числу рабочих независимо от знаменателя), оставлено ради корректности на
        // случай, если ставка снова станет непропорциональной.
