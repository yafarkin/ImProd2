...
public static class FactoryProfitabilityCalculator
{
        ...
        decimal electricityConsumptionPerOutputUnit = 0m)
    {
        ...
        // Зарплата считается на всю команду сразу (см. FinanceCalculator.CalculateSalaries) — этой
        // фабрике достаётся пропорциональная её доле рабочих часть общей суммы, а не отдельная
        // плоская ставка на неё одну (иначе оценка виджета разойдётся с тем, что реально спишет
        // TickFinanceStep). Фабрики на вынужденном простое исключены из этого пула тем же способом,
        // что и в TickFinanceStep.Run (см. фильтр !IsUnderRepair там), — их зарплата и содержание
        // идёт не через общекомандный пул, а по отдельному плоскому льготному тарифу
        // (WearStep.RunRepairTurn), не должны попадать в тот же знаменатель дважды. С 2026-08-23
        // (rebalance/2-sector-stepwise) сама командная зарплата тоже плоская (прогрессия сверх
        // порога убрана целиком) — при флэт-ставке это исключение уже не меняет итоговую долю
        // остальных фабрик (она пропорциональна числу рабочих независимо от знаменателя), оставлено
        // ради корректности на случай, если ставка снова станет непропорциональной.
