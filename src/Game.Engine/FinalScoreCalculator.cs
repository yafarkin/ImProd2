...
/// стоимости постройки, привязанной к реальному состоянию (<see cref="FactoryResidualValueCalculator"/>:
/// от <see cref="FactoryDefinitionConfig.LiquidationValueCoefficient"/> — пол, полностью убитая
/// фабрика — линейно вверх до полной <see cref="FactoryDefinitionConfig.BuildCost"/> при
/// <c>Condition=1</c>). Чистый учёт, не движение денег — не создаёт стимула «нагенерировать себе
/// выручку», в отличие от наценки/себестоимости (см. `docs/rebalance-2sector/README.md`, разбор
/// «аренда vs себестоимость»). R&amp;D не учитывается (расход, ценность уже проявилась в
/// производстве, а не в отдельном учёте). Чистая функция — не мутирует ни команду, ни себестоимости;
/// можно звать в любой момент сессии, не только по её завершении.
/// </summary>
