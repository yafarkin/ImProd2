...
/// калибровки, кроме двух зафиксированных пользователем правил (запрос пользователя,
/// rebalance/2-sector-stepwise, 2026-08-22): продажа системе — везде <c>Game.Engine.
/// MarketSaleCalculator.SystemSaleMarginMultiplier</c> (себестоимость + 5%, без деления по
/// уровню передела — до этого была таблица <c>MarginMultiplierByProcessingLevel</c>, убрана как
/// класс механики целиком, не только числа: показала себя источником багов — асимметрия множителей
/// между уровнями почти обнуляла маржу передела, дважды пришлось чинить вручную, step8 и step12);
/// аварийная закупка — база <see cref="EmergencyPurchaseBaseMultiplier"/> зафиксирована на 1.3
/// (себестоимость + 30%) на нейтральном (3) уровне ползунка сложности, поле осталось конфигурируемым
/// (не константа в коде, в отличие от продажи системе) — это один из откалиброванных рычагов <see
/// cref="DifficultyScaler"/>, ломать которую не входило в задачу.
/// </summary>
public sealed record EconomyConfig
{
    ...
    /// см. <see cref="EmergencyPurchasePressureMultiplierPerUnit"/>. Зафиксирован на 1.3 (см.
