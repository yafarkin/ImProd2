...
public sealed class MaterialOnStock
{
    ...
    /// <summary>
    /// Уменьшает остаток; бросает исключение, если запрошено больше, чем есть в наличии. Возвращает
    /// реальную себестоимость списанной части (по среднему на момент списания) — вызывающий код
    /// использует её, чтобы перенести реальную стоимость дальше (в новую партию продукции) или
    /// посчитать реальную прибыль от продажи.
    /// </summary>
