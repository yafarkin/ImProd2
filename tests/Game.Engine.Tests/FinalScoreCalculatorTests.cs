...
public class FinalScoreCalculatorTests
{
    ...
    {
        // С 2026-08-23 остаточная стоимость привязана к Condition, не плоская доля — у только что
        // построенной фабрики Condition=1, значит стоит полную BuildCost, не половину (раньше, при
        // плоской доле, было бы 50 = 100 * 0.5).
        var team = new Team(Ulid.NewUlid(), "Команда А1", TestGameConfig.SectorA);
