...
public class MarketCapacityHistoryCalculatorTests
{
    ...
    public void SummarizeCurrentTurn_Resets_When_A_New_Turn_Publishes_Fresh_Quotes()
    {
        ...
        log.Append(new MarketUpdated { Id = Ulid.NewUlid(), Quotes = TestGameConfig.Resolved.Raw.Economy.BaseMarketPerMaterial.ToDictionary(m => m.MaterialId, m => new MaterialQuote(m.BaseSellPrice, m.BaseCapacity)), ElectricityPrice = 1m, Turn = 1, EconomyIndex = 1m });
