...
rem Бот может взять несколько действий за один ход подряд (построить,
rem нанять, поднять R&D), пока сам не скажет "готово". Это — страховка
rem на случай, если модель не научится вовремя останавливаться, не
rem обычный режим работы.
